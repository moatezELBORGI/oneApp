    ...
    if (_currentBuildingId != buildingId) {
      ...
      // Notifier le changement de contexte
        ...
        if (currentBuildingId == null) {
            ...
            log.debug("Fallback building from DB: {}", currentBuildingId);
        }

        if (currentBuildingId == null) {
            log.warn("No building context found for user: {}", userId);
            return Page.empty();
        }

        // Utiliser l'ID utilisateur réel pour la requête avec filtrage strict par bâtiment
        Page<Channel> channels = channelRepository.findChannelsByUserIdAndBuilding(user.getIdUsers(), currentBuildingId, pageable);
        log.debug("Found {} channels for user {} in building {}", channels.getTotalElements(), user.getIdUsers(), currentBuildingId);
